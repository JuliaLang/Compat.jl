...
if VERSION < v"1.9.0-DEV.1163"
    ...
    if VERSION < v"1.1.0-DEV.590"
        _tuple_axes(x) = axes(x)
        _tuple_axes(@nospecialize t::Tuple) = (axes(t),)
    else
        const _tuple_axes = axes
    end
    ...
    _iterator_axes(x, ::Base.IteratorSize) = _tuple_axes(x)
    ...
    _vec_axis(A, ax=_iterator_axes(A)) = length(ax) == 1 ? only(ax) : Base.OneTo(mapreduce(length, *, ax; init=1))
    ...
    @inline function _stack_size_check(x, ax1::Tuple)
        if _iterator_axes(x) != ax1
            ...
            uaxN = map(UnitRange, _tuple_axes(x))
